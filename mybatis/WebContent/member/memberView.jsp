...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%-- 아이디를 전달받아 MYMEBER 테이블에 저장된 회원정보를 검색하여 출력하는 JSP 문서 --%>
<%-- => [회원변경]을 클릭한 경우 회원정보 변경 입력페이지(memberUpdateForm.jsp)로 이동 - 아이디 전달 --%>
<%-- => [회원삭제]를 클릭한 경우 회원정보 삭제 처리페이지(memberDeleteAction.jsp)로 이동 - 아이디 전달 --%>
<%-- => [회원목록]를 클릭한 경우 회원목록 출력페이지(memberDisplay.jsp)로 이동 --%>    
<%
	String id=request.getParameter("id");
	MyMemberDTO member=MyMemberXMLDAO.getDAO().getMember(id);
%>    
