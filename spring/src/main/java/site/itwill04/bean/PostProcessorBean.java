...
//BeanPostProcessor 인터페이스를 상속받은 자식 클래스
// => 다른 클래스의 대한 Spring Bean 초기화 작업을 제공하기 위한 메소드를 오버라이드 선언
// => Bean Configuration File에 등록된 모든 Spring Bean에 대한 초기화 작업 제공
public class PostProcessorBean implements BeanPostProcessor {
	public PostProcessorBean() {
		System.out.println("### PostProcessorBean 클래스의 기본 생성자 호출 ###");
	}
	
	//InitializingBean 인터페이스의 afterPropertiesSet() 메소드 호출 전 자동 호출되는 메소드
	@Override
	...
	//InitializingBean 인터페이스의 afterPropertiesSet() 메소드 호출 후 자동 호출되는 메소드
	@Override
